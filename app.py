...
solar_radiation = st.number_input('Solar Radiation' , 'Recorded in MJ/m^2')
rainfall = st.number_input('Rainfall' , 'Recorded in mm')
snowfall = st.number_input('Snowfall' , 'Recorded in cm')
season = st.selectbox('current season', df['Seasons'].unique())
holiday = st.selectbox('Holiday or not', df['Holiday'].unique())
working_day = st.selectbox('working day', df['Working Day'].unique())
